...
const AZURE_MS_LOGO_DATA_URL = 'data:image/svg+xml;base64,';
...
function createOidcIcon(provider) {
  ...
  if (provider === 'azure') {
    image.src = AZURE_MS_LOGO_DATA_URL;
  } else {
...
function oidcProviderName(provider) {
  if (provider === 'azure') {
    return 'Microsoft';
  }
...
function renderOidcPrompt(card, config, {
  ...
  const providerName = oidcProviderName(config.provider);
  const copy = document.createElement('p');
  copy.textContent = `Sign in with ${providerName} to join this session.`;

  const actions = document.createElement('div');
  actions.className = 'auth-gate-actions';

  const signInButton = document.createElement('button');
  signInButton.className = `ui-button ui-button--secondary auth-gate-button auth-gate-button--${config.provider || 'oidc'}`;
  signInButton.type = 'button';
  signInButton.append(
    createOidcIcon(config.provider),
    document.createTextNode(config.submitLabel || `Continue with ${providerName}`),
  );
